...
sites <- read_csv2("data_processed_sites.csv", col_names = T, na = "na", col_types = 
  ...
  select(cwmAbuSla, surveyYear, constructionYear, id, block, location, plot, exposition, plotAge, PC1, PC2, PC3, changeType, ffh) %>%
  mutate(n = cwmAbuSla) %>%
  ...
  group_by(plot, exposition, changeType, PC1, PC2, PC3, location, constructionYearF, constructionYear) %>%
...
VarCorr(m1e)
#1w-model
m2 <- lmer((n) ~ (exposition + PC1 + PC2 + PC3 + changeType) + 
             (1|location), sites, REML = F)
...
par(mfrow=c(2,2));
plotResiduals(main = "surveyYear", simulationOutput$scaledResiduals, sites$surveyYear)
...
plotResiduals(main = "ffh", simulationOutput$scaledResiduals, sites$ffh)
