...
### Choosen model ###
m4 <- lmer((n) ~ (exposition + side + PC1 + PC2 + PC3) + locationAbb + surveyYearF + 
             locationAbb:exposition + surveyYearF:PC2 +
             (1|plot), sites, REML = F)
...
data2 <- rename(sites, predicted = targetRichness, x = locationAbb, group = exposition)
...
pd <- position_dodge(.6)
(graph <- ggplot(data2, 
                 aes(x, predicted, ymin = conf.low, ymax = conf.high, fill = group)) +
    ...
    #facet_grid(~ group) +
    annotate("text", label = expression(italic(p)==1.9%*%10^-11), x = 14, y = 40, size = 2) +
    ...
    labs(x = "Location and restoration year", fill = "Exposition: ", y = expression(paste("Target species richness [#]"))) +
    themeMB()
)
