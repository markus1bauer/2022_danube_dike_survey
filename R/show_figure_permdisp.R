...
### Load data ###
data <- read_csv2("data_processed_sites.csv", col_names = T, na = "na", col_types = 
                     cols(
                       .default = col_guess(),
                       id = col_factor(),
                       location = col_factor(),
                       ...
                       ffh = col_factor(),
                       changeType = col_factor(c("FFH6510", "non-FFH", "better", "change", "worse", "any-FFH"))
                     )) %>%
  select(permdisp, surveyYear, constructionYear, id, block, location, plot, exposition, plotAge, PC1, PC2, PC3, changeType, ffh, graminoidCov, conf.low, conf.high) %>%
  mutate(n = permdisp) %>%
...
data2 <- rename(data, predicted = permdisp, x = changeType, group = surveyYearF)
...
(graph <- ggplot(pdata, 
    ...
    #facet_grid(~ group) +
    annotate("text", label = "n.s.", x = 5.2, y = 0.7) +
    scale_y_continuous(limits = c(0, 0.7), breaks = seq(-100, 100, 0.1)) +
    #scale_shape_manual(values = c(1,16)) +
    labs(x = "FFH type", y = expression(paste("PERMDISP"))) +
    themeMB()
)

### Save ###
setwd(here("outputs/figures"))
ggsave("figure_permdisp_(800dpi_8x7cm).tiff",
