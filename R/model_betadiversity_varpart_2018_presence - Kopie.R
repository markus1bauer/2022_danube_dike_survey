...
sites <- read_csv("data_processed_sites.csv", col_names = T, na = c("na", "NA"), col_types = 
  ...
  filter(accumulatedCov > 0 & surveyYear == 2018)
...
sites_space <- sites %>%
  select(locationAbbN, distanceRiver, riverkm, MEM2_2018)
sites_history <- sites %>%
...
#--> MEM1_2018 ~ riverkm has r > 0.7 (Dormann et al. 2013 Ecography) --> MEM1 has to be excluded

### * Calculate: Baselga presence-absence ####
beta <- adespatial::beta.div.comp(species, coef = "BS", quant = F)
beta$Note
beta$part #total = 0.326, substitution = 0.276, subsets = 0.050
...
(r2adj <- RsquareAdj(m1)$adj.r.squared) #R2adj = .411

### * forward selection ####
### Soil ###
m1 <- dbrda(beta_substitution ~ PC1soil + PC2soil + PC3soil + expositionN + sideN, 
            data = sites)
(r2adj <- RsquareAdj(m1)$adj.r.squared) # R2adj = 0.052
...
sites_soil_selected <- sites %>%
  select()
### Space ###
m1 <- dbrda(beta_substitution ~ locationAbb + riverkm + distanceRiver + MEM2_2018, 
            data = sites)
(r2adj <- RsquareAdj(m1)$adj.r.squared) #R2adj = 0.279
sel <- forward.sel(beta_substitution, 
                   sites_space,
                   adjR2thresh = r2adj,
                   nperm = 9999)
...
sites_history_selected <- sites %>%
  select(PC1constructionYear, PC3constructionYear, plotAge)

### * Variation partitioning ####
m1_substitution_varpart <- varpart(beta_substitution, sites_space_selected, sites_history_selected)
...
### Soil ###
### Space ###
m1_substitution <- dbrda(beta_substitution ~ riverkm + distanceRiver + MEM2_2018 +
                           Condition(PC1constructionYear + PC3constructionYear + plotAge),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .001
RsquareAdj(m1_substitution) # R2adj = .072
### History ###
m1_substitution <- dbrda(beta_substitution ~ PC1constructionYear + PC3constructionYear + plotAge + 
                           Condition(riverkm + distanceRiver + MEM2_2018),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .001
RsquareAdj(m1_substitution) # R2adj = .054
### riverkm ###
m1_substitution <- dbrda(beta_substitution ~ riverkm +
                           Condition(distanceRiver + MEM2_2018 +
                                       PC1constructionYear + PC3constructionYear + plotAge),
                         data = sites)
...
m1_substitution <- dbrda(beta_substitution ~ distanceRiver +
                           Condition(riverkm + MEM2_2018 +
                                       PC1constructionYear + PC3constructionYear + plotAge),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .017
RsquareAdj(m1_substitution) # R2adj = .020
### MEM2_2018 ###
m1_substitution <- dbrda(beta_substitution ~ MEM2_2018 +
                           Condition(riverkm + distanceRiver +
                                       PC1constructionYear + PC3constructionYear + plotAge),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .005
RsquareAdj(m1_substitution) # R2adj = .028
### PC1constructionYear ###
m1_substitution <- dbrda(beta_substitution ~ PC1constructionYear + 
                           Condition(riverkm + distanceRiver + MEM2_2018 +
                                       PC3constructionYear + plotAge),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .013
RsquareAdj(m1_substitution) # R2adj = .020
### PC3constructionYear ###
m1_substitution <- dbrda(beta_substitution ~ PC3constructionYear + 
                           Condition(riverkm + distanceRiver + MEM2_2018 +
                                       PC1constructionYear + plotAge),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .050
RsquareAdj(m1_substitution) # R2adj = .014
### plotAge ###
m1_substitution <- dbrda(beta_substitution ~ plotAge + 
                           Condition(riverkm + distanceRiver + MEM2_2018 +
                                       PC1constructionYear + PC3constructionYear),
                         data = sites)
anova(m1_substitution, permutations = how(nperm = 999)) #p = .012
...
m1 <- dbrda(beta_subsets ~ PC1soil + PC2soil + PC3soil + exposition + side + 
              ...
              plotAge + PC1constructionYear + PC2constructionYear + PC3constructionYear,
            data = sites)
anova(m1, permutations = how(nperm = 999)) #P = .385
(r2adj <- RsquareAdj(m1)$adj.r.squared) #R2adj = .079
