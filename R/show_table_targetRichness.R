...
### Load data ###
data <- read_csv2("table_anova_targetRichness.csv", col_names = T, na = "na", col_types = 
                     cols(
                       .default = col_guess()
                       )) %>%
...
#footnote
   tab_source_note( 
     source_note = "ANOVA of targetRichness for old dikes"
   ) %>%
...
gtsave(table, "table_anova_targetRichness.png")
