---
title: "Analysis of Bauer et al. (2023) Beta diversity on dike grasslands: <br> Spatial variation 2018"
...
&rarr; Remove biotope_area and pc3_construction_year

```{r data_selection}
sites_soil <- sites %>%
  select(pc1_soil, pc2_soil, pc3_soil, exposition_numeric, orientation_numeric)
sites_space <- sites %>%
  select(location_abb_numeric, river_distance, river_km, biotope_distance, mem2_2018)
sites_history <- sites %>%
...
m1 <- dbrda(
  beta_substitution ~
    ...
    location_abb + river_km + river_distance + biotope_distance + mem2_2018 +
...
```{r partial_space_biotope_distance}
m1_substitution <- dbrda(
